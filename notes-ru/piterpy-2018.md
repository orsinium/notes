...
Что касается странного порядка разрешения порядка наследования в Python, то тут всё оправдано. Дело в том, что все классы в python неявно наследуются от object, и этот общий родитель должен оказаться в самом конце порядка поиска атрибутов и методов. Сначала ищем в самом классе, потом во всех родителях, а потом уже реализацию по умолчанию в object.
...
А теперь я хотел бы рассказать о конкретном паттерне разделения приложения на компоненты: MVC.
