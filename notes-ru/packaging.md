...
Нужно создать [вот такой setup.py файл](https://docs.python.org/3/distutils/introduction.html#a-simple-example):
...
Но `distutils2` всё-таки оказал некоторое влияние на то, с чем мы живём сейчас. В 2009 году был написан [PEP-390](https://www.python.org/dev/peps/pep-0390/), описывающий формат `setup.cfg`. Идея была простая: сделать возможность получать метаданные проекта без его установки или исполнения скриптов. Формат cfg такое себе, потому что он совершенно не гибкий и не стандартизованный, но всё равно, это была отличная идея. PEP в 2013 году отвергли, потому что `distutils2` умер, однако в 2016 году поддержку этого формата [добавили](https://setuptools.readthedocs.io/en/latest/setuptools.html#configuring-setup-using-setup-cfg-files) в `setuptools`.

## sdist

[19:15] 3

Дальше из setup.py собирается `sdist` -- это `tar.gz` архив c кодом пакета и всеми его метаданными. Именно он скачивается с pypi. Формат развивался давно и стихийно, а потому и выглядит крайне занимательно. Внутри лежит директория с названием проекта и его версией, а в этой директории все нужные файлы, такие как README и LICENSE, папки с кодом проекта, иногда тесты, и директория `project_name.egg-info`. Давайте посмотрим, что в ней.

+ `PKG-INFO` -- основные метаданные проекта. Здесь название и версия проекта, описание, homepage и прочее. В общем, почти всё то же самое, что указывается в `setup.py`. Из интересного ключ `Provides-Extra` который перечисляет, какие у пакета есть extra зависимости. Но самое интересное -- формат файла. Это email сообщение.
...
+ `entry_point.txt` -- а вот это уже настоящий ini с точками входа в приложение. Entrypoints позволяют регистрировать консольные команды и плагины для всяких штук типа pytest и flake8.
...
+ `METADATA`. В PEP написано, что это прям то же самое, что и `PKG-INFO`. Верите? Не надо верить.
  ...
  + Появился заголовок `Requires-Dist`, теперь зависимости описаны в нём. Вроде бы, неплохое изменение, но парсить это стало сильно больнее. Всё дело в том, что extra группа для зависимостей, которая раньше была именем секции, теперь указывается как маркер.
...
Изначально он назывался CheeseShop, сейчас же обрёл более солидное название Warehouse. Ещё довольно примечательно, что в 2006-2007 годах в него всё хотели встроить cheesecake (pycheesecake.org) -- инструмент, для оценки кода по 3 критериям: Installability, Documentation, Code kwalitee (sic!). Был сайт с рейтингом всех пакетов: pypi.pycheesecake.org. Рейтинг качества кода оценивался с помощью PyLint. Он же и занимал первое место в этом рейтинге. Возможно, поэтому идею и забросили. Сейчас все ссылки уже мертвы, и я не знаю, что стало с проектом, но internet wayback machine всё помнит.
...
Давайте посмотрим, как работают существующие резолверы и научимся на их примере, как делать не надо.

Наихудший резолвер в pip, и это древняя и хорошо известная проблема. Резолвер в pip работает по принципу "first round wins". То есть сначала выберутся совместимые версии зависимостей самого проекта, потом зависимости зависимостей и так далее. Если же на каком-то этапе у нас появляется пакет, которому нужен пакет, который был зарезолвен на предудщих этапах, но его версия не совместима с тем, что нужно этому пакету -- ну, не судьба. pip выведет предупреждение и продолжит работать, как будто ничего не произошло. Есть шансы, что это будет работать, но особо надеяться на это не стоит. Избежать проблем с резолвером можно делегировав резолвинг кому-нибудь ещё. Если pip сказать конкрентные версии всего, что нужно установить, проблем не возникнет.
...
В poetry всё гораздо лучше. Это первый инструмент в мире python с умным резолвингом. В нём реализован алгоритм PubGrub используемый в Dart, и он работает прям хорошо. В Poetry всё очень странно и больно с выбором совместимой версии Python, но это уже особенности самого poetry, а не алгоритма. Я очень старался разобраться в этом алгоритме, но по нему есть всего одна статья от автора алгоритма, которая больше маркетинговая, делает много допущений, и вообще особо не несёт какой-то смысловой нагрузки. Код на Dart я читать не умею, а исходники Poetry больше поэзия, чем код. Например, модули там внутри называются masonry, puzzle, mixology, и я до сих пор не особо представляю, что это всё значит.

## DepHell

[19:34] 4

Но есть одна вещь, в которой ошибается, Poetry, PubGrub, да и вообще все умные публикации на эту тему. По крайней мере, почему это не применимо в суровой реальности Python. Для них главная задача -- сделать как можно меньше изменений в графе, то есть найти наилучшее решение за наименьшее число случившихся по пути конфликтов. Статья про PubGrub говорит о том, что Dart очень внимательно смотрит на зависимости всех релизов всех пакетов, связанных с конфликтом, и для многих случаев сразу говорит, если разрешить зависимости не получится.

Однако это очень дорогая операция. Нужно запросить зависимости каждого релиза. Причём, как мы помним, PyPI довольно часто ничего не знает о зависимостях релиза, поэтому для таких случаев нам придётся скачивать sdist, разархивировать, создавать окружение, запускать в нём setup.py... Чувствуете, да? Это тот момент, когда все ошибки прошлого собираются вместе во что-то ужасное.

Итак, в DepHell всё начнается с текущего проекта. Здесь может быть несколько проектов (я их называю roots), если мы сливаем вместе несколько файлов зависимостей, и конфликты могут случиться уже на следующем этапе. У этих вершин залочена текущая версия, потому что мы не можем посоветовать пользователю просто использовать предыдущую версию проекта.

Дальше мы, шаг за шагом, выбираем наилучшую версию для каждой зависимости. В dephell эти шаги называются layers. Особенность графа зависимостей в том, что пакеты из очередного слоя могут ссылаться на предыдущие слои. Может, даже на сам корневой проект, как знать. В отличие от pip-tools, мы не выдёргиваем такие зависимости в новый раунд, а оставляем их на месте, потому что для них зависимости в граф мы уже добавили.

Сейчас ещё немного терминов:

- **used** -- если какой-то пакет в графе зависит от данного пакета. Дальше мы увидим, как у нас могут появиться такие пакеты.
- **locked** -- если мы уже выбрали наилучший релиз для данного пакета.
- **applied** -- если мы добавили зависимости залоченного релиза в граф.

Ещё нужно рассказать про группы. На самом деле, мы лочим не определенный релиз, но группу релизов. Одна группа содержит релизы с одинаковыми зависимостями. Наилучшая группа -- та, в которой наилучший релиз. "Наилучший" по умолчанию означает "наиболее последней версии", но могут быть другие критерии. Поэтому пока в группе остаётся хотя бы один релиз, перестраивать граф не надо, и мы считаем, что конфликта нет. То есть, конфликт -- когда в выбранной группе не остаётся подходящих релизов.

## Conflict

[19:38] 3

И вот, момент истины, у нас есть конфликт! Один из выбранных релизов пакета не хочет работать с версией другого пакета, который мы уже залочили. Что же делать? Давайте по шагам.

1. В первую очередь, мы делаем unapply для пакета, который вызвал конфликт, потому что он мог быть частично применён к графу. Мы хотим иметь граф, который был до конфликта.
1. Дальше идёт мутация. На самом деле, это самая важная и сложная часть, и она реализована у меня довольно просто и топорно. На верхнем уровне абстракции, мутатор берёт все пакеты из графа, по-очереди меняет для разных пакетов группы. Каждая такая мутация прогоняется через быстрые проерки, которые показывают, насколько вероятно, что это нам поможет. Это всё только про вероятности, потому что если пакет не находится в полной изоляции от подграфа с конфликтом, есть вероятность, что изменение его требований полностью изменит ситуацию. Причём вариант с такой изоляцией я даже не рассматриваю, потому что есть пакеты типа attrs, six и requests, которые так или иначе связывают почти все существующие пакеты.
    1. Мутации. Варианты перебираются с помощью кастомной реализации product. Если product сначала перебирает последний пакет, потом предпоследний и т.д., то мы стараемся везде держаться последних групп, чтобы в получившемся результате все пакеты были наиболее актуальными.
    1. Проверки. Чем больше происходит конфликтов, тем менее требовательными становятся эти проверки. Сначала мы пытаемся мутировать только непосредственных родителей конфликта, потом ноды, влияющие на хоть каких-то родителей конфликта, а потом в отчаянии так вообще всё подряд.
1. Unapply. Мы берём все пакеты, которые изменились в данной мутации, и выдёргиваем их зависимости из графа. На самом деле, они здесь остаются, потому что они могут понадобиться позже, однако если после всего какие-то пакеты так и останутся никому не нужные, мы их просто выкинем. И так как эти дочерние классы изменились, мы точно так же делаем unapply для них. Это ещё одна особенность dephell. Мы никогда не перестраиваем граф, а только выдёргиваем из него то, что связано с изменениями в мутации. Работает это благодаря тому, что модель Constraint у зависимости помнит из каких пакетов какие констрейнты пришли.

И такие вот операции мы повторяем, пока не разрезолвим все зависимости. Если возникает конфликт, то мутаций происходит довольно много, но так как они ленивые и стараются быть поближе к желаемому результату, сетевых запросов гораздо меньше.

# Not only versions

[19:41] 2

Но всё это не только о правильных версиях пакета. О нет, как бы не так! В питоне очень много всякого напридумывали.

Я долго думал, как добавить поддержку для extra зависимостей. В итоге я их представляю отдельными нодами на графе, и это прям отличное решение. Пакеты, которые требуют эту extra, зависят на эту ноду, а зависимости самой ноды -- зависимости, описанные в данной extra.

А ещё есть маркеры, которую указывают, в каком окружении пакет нужно ставить, есть версии питона, с которыми пакет совместим. Таких мелких и сложных задач довольно много, поэтому я решил выносить их в отдельные пакеты. Всего накопилось 14 таких пакетов.
